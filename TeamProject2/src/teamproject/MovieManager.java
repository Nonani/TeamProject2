...
//			System.out.println(isBookedMoive(Integer.parseInt(id)));
			if(getMovie(Integer.parseInt(id))!=null) {
	            ...
	            pstmt.execute();
		        System.out.println("¿µÈ­°¡ »èÁ¦µÇ¾ú½À´Ï´Ù.");
			}else {
				System.out.println("¿Ã¹Ù¸£Áö ¾ÊÀº ÀÔ·Â°ªÀÔ´Ï´Ù.");
				return;
			}
...
//			e1.printStackTrace();
	...
	//¿µÈ­ ¿¹¸Å´Ü°è
	public String selectMovie(int date,int age) {             //ÀÎÀÚ·Î ¹ŞÀº ³¯Â¥¿¡ »ó¿µÁßÀÎ ¿µÈ­Áß »ç¿ëÀÚ¿¡°Ô ¼±ÅÃÀ» ¹ŞÀ½ ¿µÈ­ °íÀ¯ id¸¦ ¹Ş¾Æ¼­ ¸®ÅÏÇÑ´Ù
		Scanner scan=new Scanner(System.in);
		...
		while(true) {
			...
			if(ok==-1) {
				System.out.println("¿¬·É Á¦ÇÑÀÌ ÀÖ´Â ¿µÈ­ÀÔ´Ï´Ù.");
				continue;
			}
	...
	//¿µÈ­ ¿¹¸Å´Ü°è
	public int checkingBookNum(int m_id) {             //¸îÁÂ¼®ÀÌ ¿¹¸ÅµÇ¾ú´ÂÁö return
		int cnt=0;
	...
	public String selectNumOfPersons(int m_id) {           //¿µÈ­ ¿¹¸Å ÀÎ¿øÀ» ¸®ÅÏÇÑ´Ù
		...
		if(emptyseat==0) {
			System.out.println("³²Àº ÁÂ¼®ÀÌ ¾ø¾î ¿¹¸Å¸¦ Á¾·áÇÕ´Ï´Ù.");
			return "x";
		}
		while(true) {
			...
			if(Integer.parseInt(idx)<1||Integer.parseInt(idx)>64) {
				...
				System.out.println("ÇØ´ç ÀÎ¿ø¼ö°¡ ÀÜ¿© ÁÂ¼®º¸´Ù ¸¹½À´Ï´Ù.");
				continue;
			}else
				break;
		}
...
//È®ÀÎ¿ë			System.out.println(tm.t_list.get(i).getMovie().getId()+","+tm.t_list.get(i).getS_id()+","+id);
	...
	public int book(String u_id,TicketManager tm,UserManager um,MovieManager mm,int u_age) {
		...
		String idx=selectMovie(Integer.parseInt(date),u_age);  //¿µÈ­ id
		if(idx.contentEquals("x")||idx.contentEquals("X")) {
			return 0;
		}
...
//È®ÀÎ¿ë	System.out.println("¿µÈ­ id : "+idx+"index : "+idx_2);
	...
	public int checkBookandCancel(String u_id) {
		...
		if(arr[0]==-1) {
			System.out.println("¿¹¸ÅµÈ Á¤º¸°¡ ¾ø½À´Ï´Ù.");
			return 0;
		}
		while(true) {
		...
		System.out.println("¿µÈ­ Ãë¼Ò°¡ ¿Ï·áµÇ¾ú½À´Ï´Ù.");
		return 0;	
	}
	//¿µÈ­ °Ë»ö ´Ü°è
	...
	public int Search() {
		...
		while(escape) {
			...
			case "2" : {
				...
				String idx_2;
				while(true) {
					System.out.println("¸î ¼¼ °ü¶÷ ¿µÈ­¸¦ Ã£À¸½Ã³ª¿ä? [ÀüÃ¼ÀÌ¿ë°¡|7¼¼|12¼¼|15¼¼|19¼¼],ÀüÃ¼ÀÌ¿ë°¡´Â 0À» ÀÔ·ÂÇØÁÖ¼¼¿ä");
					System.out.println("[Âü°í : ¸ŞÀÎÈ­¸éÀ¸·Î µ¹¾Æ°¡·Á¸é x(X) ÀÔ·Â]");
					System.out.print("input : ");
					idx_2=scan.nextLine();
					if(idx_2.contentEquals("x")||idx_2.contentEquals("X")) {
						...
						System.out.println("¿Ã¹Ù¸£Áö ¾ÊÀº ÀÔ·Â°ªÀÔ´Ï´Ù.1");
						continue;
					}
					if(Integer.parseInt(idx_2)!=0&&Integer.parseInt(idx_2)!=7&&Integer.parseInt(idx_2)!=12&&Integer.parseInt(idx_2)!=15&&Integer.parseInt(idx_2)!=19) {
						System.out.println("¿Ã¹Ù¸£Áö ¾ÊÀº ÀÔ·Â°ªÀÔ´Ï´Ù.2");
						continue;
					}
					break;
